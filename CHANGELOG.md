...
- **Four MCP hosts per project:** stdio (Cursor), public HTTP, passthrough HTTP (client header → upstream), OAuth HTTP (Open WebUI / enterprise IdP demos)
- **Upstream auth block:** `auth { in: header|query, name, prefix }` maps MCP credential to API header or query param on protected tools
- **Access control:** `access: public | protected` plus optional hooks — `verifyCredential`, `authorize`, `prepare` (see core2ai auth pipeline)
