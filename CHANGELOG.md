...
- **Publisher:** Marketplace / VSIX publisher id is `toolfactorydev` (extension id `toolfactorydev.vscode-api2ai`)

### Upgrade notes

- Reinstall the VSIX (or update from Marketplace) so the installed extension id matches demo `extensionIdPrefix`
...
- **MCP demo icons:** optional `icon` in catalogs → `--icon` for cakes/banking/spaceflight-news; omit for Tool Factory core fallback (no product `fallback.png`)
...
- **Demo scripts:** utility modules live under `generated/api2ai/scripts/` (synced from core); hand copies under `scripts/generated/` removed
...
- Pin refresh: `npm run sync:core2ai-pin:npm` (or install VSIX), then regenerate so `generated/api2ai/scripts/**` is current
- Existing Test workspaces created with a broken copy: recreate via **Create demo workspace**, or copy `generated/api2ai/scripts/` from the extension demos bundle

---

## [1.0.0-rc.4] - 2026-07-10

Strict MCP arg validation, LLM-facing type hints in tool descriptions, inspector-only `mcp:inspect`. Pins `@toolfactory.dev/core` **1.0.0-rc.8** from npmjs.

### Changed

- **Invoke:** pass MCP path/query/body args through without runtime coercion (preserves string values such as leading zeros)
- **Codegen:** `(type: …)` and `(example: …)` in OpenAPI parameter descriptions and tool `Parameters:` sections
- **`mcp:inspect`:** opens MCP Inspector only — prerequisite `npm run start:all` (removed `--with-deps` / host startup)

### Upgrade notes

- Regenerate after pin refresh: `generate:all` + `build:generated`; restart MCP servers in Cursor
- Strict Zod at MCP boundary: numeric/boolean args must match schema types (no `"42"` for integer fields)
