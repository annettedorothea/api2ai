...
export function renderMcpHostAdapterBlock(authKind: 'none' | 'credential', typescript: boolean): string {
    const credentialResolve = renderCredentialResolve(authKind);
    const resolveHostContextSignature = typescript ? 'resolveHostContext(): ApiHostContext' : 'resolveHostContext()';
