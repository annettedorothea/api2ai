...
const bundlePath = path.join(extOutDir, 'embed-api2ai', 'cli.cjs');
const embedPkgDest = path.join(extOutDir, 'embed-api2ai', 'package.json');
const mcpDest = path.join(extOutDir, 'embed-api2ai', 'resources', 'mcp-serve-emitted.mjs');
...
await esbuild.build({
    ...
    define: {
        __API2AI_CLI_BUNDLE_VERSION__: JSON.stringify(cliVersion),
    },
    external: [
        'vscode-jsonrpc',
