#!/usr/bin/env bash
# Validates prerequisites when the user triggers the api2ai MCP smoke-test shortcut.
set -euo pipefail
...
if ! printf '%s' "$trimmed" | grep -qiE '^(api2ai[[:space:]]+)?/?test(-all|:all)$'; then
    allow
fi
...
if [[ ! -f .cursor/skills/api2ai-test-all-mcp/SKILL.md ]]; then
    deny "test-all: Skill .cursor/skills/api2ai-test-all-mcp/SKILL.md fehlt."
fi
...
if ! node scripts/check-mcp-ready.mjs >&2; then
    deny "test-all: HTTP-MCP-Ports nicht erreichbar. npm run start:mcp (Fixtures laufen?) oder npm run start:all."
fi
