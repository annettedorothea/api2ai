 ...
 * Fixtures/IdPs must already be running (start:all or start:fixtures).
 */
import { waitForForegroundServiceShutdown } from '../generated/api2ai/scripts/foreground-lifecycle.mjs';
import {
    demosRoot,
    generateAndCompile,
    prepareWorkspaceEnv,
...
async function main() {
    ...
    console.log(`[${logTag}] Ctrl+C stops MCP processes started here (fixtures keep running).`);
