...
function resolveEmbedHome(cliPath) {
    ...
    if (path.basename(parent) === 'embed-api2ai') {
        return parent;
    }
    return undefined;
}
...
function findInstalledExtensionCli() {
    ...
    for (const root of roots) {
        ...
        for (const entry of readdirSync(root, { withFileTypes: true })) {
            ...
            if (!entry.name.startsWith('api2ai.vscode-api2ai-')) {
                continue;
            }
            candidates.push(path.join(root, entry.name, 'out', 'embed-api2ai', 'cli.cjs'));
        }
    }
    candidates.sort();
...
function resolveCliSpawn() {
    ...
    throw new Error(
        [
            'api2ai CLI not found.',
            '• Install the api2ai VS Code/Cursor extension (VSIX), or',
            '• Run from the api2ai monorepo with packages/extension/demos as cwd, or',
            '• Set API2AI_CLI to cli.js / cli.cjs.'
        ].join('\n')
    );
}
...
if (!dslRelative || !outRelative) {
    console.error('Usage: node scripts/generate.mjs <file.api2ai> <generated/tools/out.ts>');
    process.exit(1);
}
...
const env = embedHome ? { ...process.env, API2AI_EMBED_HOME: embedHome } : process.env;
