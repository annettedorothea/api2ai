...
function main() {
    ...
    if (!name || !HTTP_DEMOS[name]) {
        ...
        console.error('stdio-only (secrets): api2ai-tmdb, api2ai-github in mcp.json.');
        process.exit(1);
    }
    ...
    if (name === 'mock-api') {
        console.warn(
            '[mcp-http:mock-api] Protected mock tools: use stdio api2ai-mock-api (envFile). HTTP has no auth headers in mcp.json.'
        );
    }
