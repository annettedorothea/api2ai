/**
 * MCP catalog entries for api2ai demos (shared by start.mjs).
 */
...
function missingOptionalSecretEnvNames(demo, env) {
    return [...new Set([demo.authExpectedEnv, demo.authEnv].filter(Boolean))].filter((name) => !env[name]?.trim());
}
...
export function buildHttpMcpCatalogEntry(name, env, options = {}) {
    ...
    if (demo.authExpectedEnv) {
        const headerName = demo.mcpAuthHeaderEnv
            ? env[demo.mcpAuthHeaderEnv]?.trim() || demo.mcpAuthHeaderEnv
            : 'x-api-token';
...
export function buildOAuthMcpCatalogEntry(name, env, options = {}) {
    ...
    return {
        ...
        oauthNote: 'Cursor: Sign-in on MCP server (alice/bob/admin for bookings).'
    };
}
 ...
 * @param {string[]} names
 * @param {NodeJS.ProcessEnv} env
 * @param {Map<string, { status?: 'running' | 'warning' | 'skipped', skipReason?: string }>} [statusByName]
 */
