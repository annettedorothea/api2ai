...
| `integration/bookings-api-oauth-mcp-http.test.ts` | bookings-oauth MCP path: HTTP (oauth) + oidc JWKS            |
| `integration/cakes-api-oauth-mcp-http.test.ts`    | cakes OAuth opaque + upstream JWT                            |
| `integration/banking-api-oauth-mcp-http.test.ts`  | banking-oauth HTTP (oauth) + verifyCredential token exchange |
| `integration/open-meteo-mcp-http.test.ts`         | public HTTP MCP host codegen (open-meteo is stdio in matrix) |
